...
Why this script, not three separate manual builds: each engine's own
package_app.py already runs `publish.py`, which refuses to package data it
cannot sanity-check (see publish.py's own docstring) -- so a stale or
half-built pipeline fails loudly here, one command in, rather than
surfacing later as three exes that don't agree with each other.

This packages whatever is CURRENTLY in each project's data/models -- run
the ETL -> train -> build_book pipeline (and look at the numbers) for both
products first if you want this build to ship anything newer than the last
publish. See backend/package_app.py and NewBusiness/backend/package_app.py.
"""
...
Your data (stage edits, uploads, hand-made corrections) is saved to
...
folder (renewal_engine.log, nb_engine.log).
"""
...
def main() -> None:
    print("== 1/4  build the renewal engine (publishes + stages current renewal data) ==")
    sh(f'"{sys.executable}" package_app.py', cwd=RENEWAL_BACKEND)
    renewal_exe = RENEWAL_BACKEND / "dist" / "HorizonRenewalEngine.exe"
    if not renewal_exe.exists():
        sys.exit(f"FAILED: {renewal_exe} was not produced")

    print("\n== 2/4  build the New Business engine (publishes + stages current NB data) ==")
    ...
    print("\n== 3/4  stage both engine exes for embedding (gateway/build/embed) ==")
    ...
    print("\n== 4/4  build the gateway launcher (embeds both engine exes inside it) ==")
